...
class BellBrent3 implements IAgent
{
	static class ShadowOpponent implements IAgent
	{
		static int iter;
	...
	public void update(Model m) {
		for(int i = 0; i < m.getSpriteCountSelf(); i++) {
			...
			{
				int doAction = tryAllOptions(m, i);
				//Shoot at flag
				if(doAction == 0)
				{
				...
				{
					moveToPosition(m, i, Model.XFLAG_OPPONENT, Model.YFLAG_OPPONENT);
				}
			}
		}
	}
	
	public int tryAllOptions(Model m, int i)
	{
		int bestAction = 4;
		double bestUtility = Double.MIN_VALUE;
		for(int sim = 0; sim < 5; sim++) { // try all candidate actions
			// Fork the universe and simulate it for 10 time-steps
			Controller cFork = m.getController().fork(new Shadow(sim), new ShadowOpponent());
			for(int j = 0; j < 10; j++)
				cFork.update();

			// See how good the option worked
			double utility = computeUtility(cFork);
	...
	public double computeUtility(Controller m)
	{
	...
	private void moveToPosition(Model m, int sprite, float x, float y) {
		State[][] costMap = AStar.search(
				terrainMap, 
				terrainMap[(int) x / 10][(int) y / 10], 
				terrainMap[(int) x / 10][(int) y / 10]);
		...
		if (costMap != null) {
			...
			if (prevState != null) {
				m.setDestination(sprite, prevState.x * 10, prevState.y * 10);
			}
		}
	}
